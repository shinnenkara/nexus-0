...
1. **Pre-flight & Fetch State:**
    ...
    - *Validation:* If the output does not contain `## Description:` and `### Changes:`, STOP and inform the user the template is broken.
...
2. **Gather Delta Context (Token-Optimized):**
    ...
    - *Only* if the commit messages are too vague to determine what technical changes occurred, run a filtered diff: `git diff main...HEAD -- . ':(exclude)*lock.json' ':(exclude)dist/*' ':(exclude)*.svg'`
